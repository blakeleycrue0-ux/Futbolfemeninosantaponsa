/*
  ffib-sync.js — Netlify Function (Scheduled)
  ============================================================================
  AVISO IMPORTANTE — LEER ANTES DE TOCAR ESTE FICHERO
  ----------------------------------------------------------------------------
  La FFIB (ffib.es) no ofrece una API pública. Esta función hace SCRAPING de
  las páginas HTML públicas de resultados/clasificación que la federación
  publica en su web (no existe ningún acuerdo ni integración oficial con la
  FFIB). Eso implica:

    - El HTML de ffib.es puede cambiar sin aviso. Si un día esta función
      empieza a fallar o a guardar datos vacíos/incorrectos, lo primero que
      hay que mirar es si han cambiado la maquetación de las páginas
      NFG_CmpJornada / NFG_CmpClasificacion y ajustar los selectores de
      cheerio de más abajo.
    - No se debe llamar a la FFIB en cada visita de un usuario a la web.
      Esta función se ejecuta por cron (Netlify Scheduled Functions,
      configurado en netlify.toml) cada pocas horas, y el resultado se cachea
      en Supabase (tablas `matches` y `ffib_standings`). Las páginas públicas
      SIEMPRE leen de Supabase, nunca de la FFIB directamente.
    - Si el scraping falla, esta función NO borra los datos ya guardados:
      deja el último dato cacheado tal cual y registra el fallo en
      `ffib_sync_log` para poder diagnosticarlo desde el panel admin. Como
      último recurso, el panel admin permite introducir resultados a mano
      (tabla `matches` con fuente = 'manual').

  Variables de entorno requeridas:
    SUPABASE_URL
    SUPABASE_SERVICE_ROLE_KEY   (clave de servicio, NO la anon key: esta
                                 función necesita saltarse RLS para escribir)
    ...
    FFIB_JORNADA_URL_TEMPLATE   (opcional) plantilla de URL de jornada con
                                 el marcador {cod}. Por defecto usa el
                                 patrón documentado de ffib.es.
    FFIB_CLASIFICACION_URL_TEMPLATE (opcional) idem para la clasificación.

  Puede invocarse manualmente (botón "Sincronizar ahora" en el panel admin)
  además de por el cron — ambos caminos ejecutan el mismo código.
  ============================================================================
*/
